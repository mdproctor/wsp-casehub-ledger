...
- `KubernetesVaultTokenSource` → Vault `kubernetes`
- `JwtVaultTokenSource` → Vault `jwt`
...
**loginRequestBody():** `{"role":"<role>","jwt":"<jwtSupplier.get()>"}`
...
4. Update Javadocs: `role()` → "Role for Kubernetes or JWT auth methods." `jwtPath()` → "Path to JWT file for KUBERNETES or JWT auth methods." `jwt()` → "Static JWT string for JWT auth method. Mutually exclusive with jwt-path."
...
Update `KUBERNETES` case to apply file path default explicitly: `authConfig.jwtPath().orElse("/var/run/secrets/kubernetes.io/serviceaccount/token")`.
...
| `signing/vault-transit/src/test/java/.../JwtVaultTokenSourceTest.java` | New |
...
| `signing/vault-transit-quarkus/src/main/java/.../VaultTransitAgentSigner.java` | Add JWT case, update KUBERNETES case |
| `signing/vault-transit-quarkus/src/test/java/.../VaultTransitAgentSignerIT.java` | Add JWT auth tests |

## Out of scope

- Browser-based OIDC flow (two-step auth URL + callback) — different use case, file a separate issue if needed
